...
# Pre-flight. A brand with no values sends no brand filter, and HubSpot then
# cheerfully returns the entire portal. Every tab would show the same number
# and every percentage would be a percentage of the whole database. That is a
# wrong answer wearing the costume of a right one, so we refuse to fetch.
if mode == "property":
    unmapped = [b["name"] for b in active_brands if not b.get("values")]
    if unmapped:
        ui.header(
            title=APP_TITLE,
            subtitle=APP_SUBTITLE,
            badge="NOT CONFIGURED",
            meta="brand filter inactive",
            stale=True,
            logo=HEADER_SRC,
        )
        prop = active_brand_property.get(OBJECTS[0]) or "(none picked)"
        ui.alarm(
            "BRAND FILTER INACTIVE",
            "No values are mapped for: <b>"
            + ", ".join(unmapped)
            + "</b>.<br><br>"
            "Without them every brand tab would count your <b>entire portal</b>, "
            "so every tab would show the same total and every percentage would be "
            "a share of the whole database rather than of that brand. Numbers are "
            "withheld until this is fixed.<br><br>"
            "<b>Fix it:</b> open <b>Brand setup</b> in the sidebar. The brand "
            f"property is currently <code>{prop}</code>. Tick which of its values "
            "belongs to each brand, and the dashboard will load.<br><br>"
            "If your contacts carry no brand property at all, switch <b>How brands "
            "are separated</b> to <b>No split</b> to see the portal as one bucket, "
            "then create a brand property in HubSpot and backfill it."
        )
        st.stop()
...
# Post-flight. The pre-flight check catches an empty value list. This catches
# the subtler failure: values are mapped, but they do not match what is actually
# stored, or they match everything. Either way the giveaway is that the brands
# stop looking like different brands.
if len(active_brands) > 1 and not rollup.empty:
    ...
    if counts.nunique() == 1 and counts.iloc[0] > 0:
        ui.alarm(
            "EVERY BRAND RETURNED THE SAME RECORD COUNT",
            f"All {len(active_brands)} brands came back with exactly "
            f"<b>{int(counts.iloc[0]):,}</b> records. Three different brands "
            "almost never have identical totals, so the brand filter is very "
            "likely matching everything.<br><br>"
            "Check <b>Brand setup</b>: the property may be one that every contact "
            "shares, or the mapped values may be wrong. The numbers below are "
            "probably counting your whole portal three times over."
        )

    empty = [name for name, n in counts.items() if n == 0]
    if empty:
        ui.alarm(
            "A BRAND MATCHED ZERO RECORDS",
            "Zero contacts matched: <b>" + ", ".join(empty) + "</b>.<br><br>"
            "The value is almost certainly stored differently in HubSpot than in "
            "the mapping. HubSpot saves the internal value, which often differs "
            "from the label shown in the UI. Reopen <b>Brand setup</b> and pick "
            "from the dropdown rather than typing."
        )
