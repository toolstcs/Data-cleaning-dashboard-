...
## How a contact gets a brand

There is no "brand" field in your HubSpot. A contact belongs to a brand when
that brand's **marker column is not empty**:

| Brand | Marker column | Rule |
|---|---|---|
| The Commerce Shop | `e-Commerce Technologies` | not empty |
| BinaryWorks | `Drupal Partners (CMS)` | not empty |
| ConversionBox | `ConversionBox Competitors` | not empty |

In HubSpot that is the `HAS_PROPERTY` operator, a presence check, not an
equality check. Because the three columns are independent, **a contact can be in
two brands at once**, and the overlap panel counts exactly how many are.

The app shows the resolved internal name on each brand card ("in this brand when
`drupal_partners_cms_` is filled") so a mis-resolution is visible at a glance
rather than silently wrong.

### The substring bug, and why it cannot come back

In the previous build, `Webinar - TCS/DrupalPartners` got substring-matched
instead of `Drupal Partners (CMS)`, and every BinaryWorks contact collapsed into
"Other CMS". The resolver here matches the internal name first, then the **exact
normalised label**, and never a substring. There is a test that puts the decoy
column in a fake portal and asserts BinaryWorks does not bind to it.

### The BinaryWorks undercount

Watch this on the first run. In the old lead dashboard, only about **1,400**
contacts had `Drupal Partners (CMS)` filled while roughly **138,000** carried
`TAG = Drupal`. Marker-only detection undercounts BinaryWorks by two orders of
magnitude if that is still true.

If BinaryWorks comes back far too small, widen the rule in `config.py`:
...
The brand then becomes "marker filled **OR** tag matches".

**Trade-off:** the overlap maths has to express `(A1 or A2) AND (B1 or B2)` as a
cartesian product of HubSpot filter groups, and HubSpot allows only 5 groups per
search. One rule per brand makes the three-way intersection a single group. Two
rules per brand makes it 8 groups, over the limit, so the overlap panel will say
it cannot be calculated rather than print a wrong number.

### Guards

A brand with no resolved marker would send no filter, and HubSpot would return
the entire portal. The app **refuses to render numbers** in that state and shows
a `BRAND MARKER MISSING` banner instead, because a wrong number that looks right
is worse than no number. Two more checks run after every fetch: all brands
returning an identical count, and any brand returning zero.
...
A full refresh is **37 calls**: 3 brands times (1 record count + 10 fields), plus
4 for the overlap (three pairs and the triple). About 13 seconds, then it caches
for 30 minutes.
...
The three real logos are already in place, pulled from thecommerceshop.com,
thebinaryworks.com and conversionbox.ai. All three are dark wordmarks, so they
read correctly on the white theme. Switching `THEME` to `"dark"` would need the
reversed variants.

`.svg` is preferred, then `.png`, `.webp`, `.jpg`. The app checks in that order
and stops at the first hit, so dropping in `tcs.png` does nothing while
`tcs.svg` still exists. Files are read from disk and inlined as base64 data
...
`THEME = "light"` in `config.py` gives the white background. `"dark"` gives the
original void background.

If you flip it, **also flip `base` in `.streamlit/config.toml`**, which controls
Streamlit's own chrome (sidebar, widgets, tooltips). Those two files cannot read
each other, so they have to be changed together. The header comment in
`config.toml` lists the exact values.

The light palette is not the dark one inverted. Neon colours that glow against
`#070b14` turn illegible on white, so the text greens and reds are darkened and
the glow shadows on the status dots become rings.
...
is one field on one contact, so 5,000 contacts times 10 fields is 50,000 cells.

**TOTAL RECORDS is the unique contact count**, not the sum of the three brand
tabs. Adding the tabs double counts anyone with two marker columns filled. The
overlap panel shows the gap: every region of the Venn diagram, the summed total,
the unique total, and how many contacts sit in two or more brands.
...
- **HubSpot allows 5 filter groups per search.** That is what caps `extra_rules`
  and what makes the overlap maths refuse to run when the rules get too complex.
...
metrics.py              Property resolution, filters, counting, overlap maths
ui.py                   Light and dark palettes, CSS, HTML card renderers
