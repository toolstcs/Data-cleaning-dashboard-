...
## Read this first: the brand filter

If every brand tab shows the **same record count**, the brand filter is not
being applied and the dashboard is counting your entire portal once per brand.
The percentages are then percentages of your whole database, not of that brand.

This happens when a brand has no **values** mapped in **Brand setup**. With no
values the code sends no brand filter, and HubSpot returns everything.

The app now refuses to render numbers in that state. It shows a
`BRAND FILTER INACTIVE` banner instead, because a wrong number that looks right
is worse than no number. Two further guards run after every fetch:

- **All brands returned an identical count** -> the mapped values are probably
  matching everything, or the property is one every contact shares.
- **A brand matched zero records** -> the value is stored differently in HubSpot
  than in your mapping. HubSpot saves the *internal* value, which often differs
  from the label in the UI. Pick from the dropdown rather than typing.

**No brand property in HubSpot at all?** Then there is nothing to filter on and
you have three options: create a custom dropdown property and backfill it; use
HubSpot Business Units (`hs_all_assigned_business_unit_ids`) if you have the
add-on; or run **No split** and treat the portal as one bucket until a brand
field exists.
