...
.marker-tag{font-family:var(--font-mono);font-size:9px;color:var(--text-3);background:var(--track);border-radius:3px;padding:2px 6px;display:inline-block;margin:8px 0 2px;}
.ov-badges{display:flex;flex-wrap:wrap;gap:5px;margin:6px 0 2px;}
.ov-badge{font-size:9.5px;font-family:var(--font-mono);color:var(--cyan);background:var(--badge-bg);border:1px solid var(--badge-br);border-radius:4px;padding:2px 6px;}
.ov-badge.none{color:var(--text-3);background:var(--track);border-color:var(--border);}

.ov-wrap{background:var(--panel);border:1px solid var(--border);border-radius:12px;padding:16px 20px 18px;margin-bottom:18px;box-shadow:var(--shadow);}
.ov-bar{display:flex;height:14px;border-radius:7px;overflow:hidden;background:var(--track);margin-bottom:14px;}
.ov-seg{height:100%;}
.ov-row{display:grid;grid-template-columns:14px 1fr 90px;align-items:center;gap:10px;padding:5px 0;font-size:12px;border-top:1px solid var(--hair);}
.ov-row:first-of-type{border-top:none;}
.ov-swatch{width:10px;height:10px;border-radius:3px;}
.ov-num{text-align:right;font-family:var(--font-mono);font-weight:700;}
.ov-foot{display:flex;justify-content:space-between;gap:16px;margin-top:12px;padding-top:12px;border-top:1px solid var(--border);flex-wrap:wrap;}
.ov-stat{min-width:120px;}
.ov-stat-l{font-size:9px;letter-spacing:0.5px;color:var(--text-3);margin-bottom:3px;}
.ov-stat-v{font-size:17px;font-weight:700;font-family:var(--font-mono);}
...
def brand_card(
    ...
    body = []
    marker = str(rollup_row.get("marker") or "")
    if marker:
        body.append(
            f'<div class="marker-tag">in this brand when {marker} is filled</div>'
        )
    if shares is not None:
        chips = "".join(
            f'<span class="ov-badge{"" if n else " none"}">also in {name}: {n:,}</span>'
            for name, n in shares
        )
        body.append(f'<div class="ov-badges">{chips}</div>')
    for obj in objects:
...
SEG_COLOURS = ["var(--cyan)", "var(--fill-green)", "var(--fill-amber)",
               "var(--text-2)", "var(--text-3)", "var(--fill-red)", "var(--panel)"]


def overlap_panel(summary: dict, names: dict[str, str], expressible: bool = True) -> None:
    """
    Every region of the Venn diagram, drawn as one stacked bar plus a key.

    The point of this panel: adding the three brand tabs together double counts
    anyone whose contact record has two marker columns filled. This shows how
    many that is.
    """
    if not expressible:
        alarm(
            "OVERLAP NOT CALCULABLE",
            "One or more brands use <code>extra_rules</code>, which makes the "
            "three-way intersection need more than HubSpot's limit of 5 filter "
            "groups. Rather than print a wrong number, the overlap is skipped. "
            "Drop back to one rule per brand to get it back."
        )
        return

    regions = summary["regions"]
    unique = summary["unique"]
    ordered = sorted(regions.items(), key=lambda kv: (len(kv[0]), -kv[1]))

    segments, rows = [], []
    for i, (keys, n) in enumerate(ordered):
        if n <= 0:
            continue
        colour = SEG_COLOURS[i % len(SEG_COLOURS)]
        pct = (n / unique * 100) if unique else 0
        label = " + ".join(names[k] for k in sorted(keys, key=list(names).index))
        if len(keys) == 1:
            label += " only"
        segments.append(
            f'<div class="ov-seg" style="width:{pct:.2f}%;background:{colour};" '
            f'title="{label}: {n:,}"></div>'
        )
        rows.append(
            '<div class="ov-row">'
            f'<span class="ov-swatch" style="background:{colour};"></span>'
            f"<span>{label}</span>"
            f'<span class="ov-num">{n:,}</span>'
            "</div>"
        )

    _html(
        '<div class="ov-wrap">'
        f'<div class="ov-bar">{"".join(segments)}</div>'
        + "".join(rows)
        + '<div class="ov-foot">'
        f'<div class="ov-stat"><div class="ov-stat-l">UNIQUE CONTACTS</div>'
        f'<div class="ov-stat-v">{unique:,}</div></div>'
        f'<div class="ov-stat"><div class="ov-stat-l">SUM OF BRAND TABS</div>'
        f'<div class="ov-stat-v txt-a">{summary["summed"]:,}</div></div>'
        f'<div class="ov-stat"><div class="ov-stat-l">IN 2+ BRANDS</div>'
        f'<div class="ov-stat-v txt-r">{summary["in_multiple"]:,}</div></div>'
        f'<div class="ov-stat"><div class="ov-stat-l">DOUBLE COUNTED</div>'
        f'<div class="ov-stat-v txt-r">{summary["duplicated"]:,}</div></div>'
        "</div></div>"
    )
